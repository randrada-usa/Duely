...
Current milestone: **M8 — Gemini-Assisted Extraction** is in progress as an internal-development and friends-preview feature; production builds keep the client entry point disabled. **M5 — On-Device OCR and Editable Review** remains open only for the broader beta-scan accuracy gate. **M6 — Supabase Backend and Authentication** and **M7 — Cloud Synchronization** are complete for the beta scope.

Current outcome: M5 implementation is complete and its beta-scan accuracy gate remains open. The Android development build now runs bundled ML Kit OCR fully on device, keeps raw text and the temporary image transient, parses conservative task candidates, flags uncertainty per field, handles no-text/multiple-assignment recovery, and saves only an editable user-confirmed task with field provenance. A deterministic post-OCR parser harness covers 24 synthetic English, Filipino, and mixed-language cases. The 12-image synthetic Android pack completed at 99.5% aggregate token accuracy and 12/12 parser-ready results after a targeted, review-visible correction for ML Kit reading `August` as `Auqust`. Clearly printed deadlines passed 6/6, missing-deadline avoidance passed 3/3, and ambiguous-deadline flagging passed 3/3. These small synthetic results validate the harness but do not certify beta-scan accuracy. The UI/UX revision is underway: the shared palette, typography, surfaces, bottom navigation, Home screen, Tasks collection, the complete Scan intake/progress/editable-review flow, Calendar month and selected-day states, Profile identity and settings, read-first Task Details with explicit edit/completion/delete actions, manual Add Task editor, empty state, buttons, editable task form, and reusable task card now follow the local DuelyPages direction without adding unsupported productivity metrics or removing existing task controls. The shared manual, edit, and extraction-review form now keeps its primary save action in a persistent bottom bar while the fields scroll, matching the local TaskDetails/ExtractionReview direction and preventing long forms from burying confirmation. Extraction review also presents a live remaining-warning count and visually highlights uncertain text and deadline controls; the count clears field by field only after student input. Home, Tasks, all primary Scan states, Calendar empty and populated dates, Profile reminder and account/backup states, Task Details read/edit states, Add Task, and the revised text-extraction review are visually verified on the Pixel 7 emulator; the focused Scan flow hides the primary navigation and respects Android safe areas until the student leaves or finishes it. A cross-screen Pixel pass at 130% and 150% Android font scaling verified the primary destinations and task flows after responsive fixes to Home schedule controls, Calendar month navigation, long profile text, and Task Details actions. Camera explanation, denial, repeated denial, settings recovery, and alternate-gallery actions were also verified at 150% text, and progress states now expose explicit accessibility semantics.

Next recommended task: finish the remaining Gemini quota-exhaustion and forced live-provider-fallback checks, then continue the local DuelyPages/DuelyIcons UI/UX consistency pass while preserving the verified task and reminder behavior. The authenticated Edge Function is deployed with JWT verification, a 20-second upstream timeout, and an atomic cancellation/refund path. A synthetic Pixel 7 gallery scan passed explicit cloud-processing consent, text-only Gemini extraction, editable review, and the configurable five-use allowance. Pixel cancellation now visibly returns to the on-device review without consuming another allowance; an initial cancellation race was discovered and corrected with an authenticated cancellation tombstone that prevents a late reservation or refunds a just-completed request. The source image remained local and no test task was saved. The local Gemini credential also passes structured synthetic extraction with Gemini 3.1 Flash-Lite, and an adversarial two-assignment fixture was flagged as multiple without following or reproducing an injected instruction. Client contract tests cover success, cancellation, disabled, consent-required, quota, provider failure, and malformed-response recovery. Development and internal preview builds expose optional Gemini assistance; production builds keep the client entry point disabled. The Pixel 7 TalkBack pass verifies clean labels and selected/disabled states across all five primary destinations, manual Add Task, Task Details, task cards, Calendar dates/empty state, notification explanation recovery, blank validation, and unsaved-change protection. Decorative icon glyphs no longer leak into those control names. Passwordless school-email authentication and the verified-school badge are deliberately deferred. The real Nunito and Inter font families now load before app startup. Pixel 7 verification confirmed local task retention through sign-out, successful Google sign-in to a second account, separate cloud ownership, an explicit first-backup prompt without silent copying, explicit empty-device restore, and a keep-data uninstall/reinstall that retained all 47 app-data files, the local task, and the signed-in Google session. A synthetic signed-in gallery scan also passes OCR, editable review, save, permission recovery, Android reminder registration, cold restart, confirmed airplane-mode restart, and cleanup. Startup reconciliation recreates a single reminder after Android force-stop clears its alarm. Queued offline retry, ongoing subject/reminder synchronization, and the corrected 12-image OCR pack are also live-verified on Android.

### September 23 backend checkpoint

This dated checkpoint supersedes the earlier "Next recommended task" in the
long M8 progress narrative above.

The hosted Duely project has the verified Plus allowance migration and version 9
of `gemini-extract` deployed with JWT verification. The new allowance function is
executable only by `service_role`; unauthenticated function requests return 401.
Migration filenames now match the two earlier hosted cancellation migrations.
This is **not** an end-to-end Plus pass: the server-only RevenueCat key and an
authenticated test-purchase/expiry check remain unverified. Real-student Gemini
processing remains gated and must not be enabled without the privacy review.
Google sign-in and Guest remain the beta choices; school-email login is deferred.
...
- [x] Navigation works with TalkBack labels and Android back behavior.
...
- [x] Recreate task reminders after a cold app restart clears Android alarms.
...
Status: `[~] In progress — authenticated hosted success, timeout protection, and cancellation/refund verified; quota exhaustion and forced live fallback checks remain`
...
- [~] Handle cloud timeout, service failure, quota exhaustion, and offline mode. Timeout protection is deployed; cancellation/refund is live-verified on Pixel; quota and failure recovery are contract-tested but still need forced live checks.
...
- [x] Verify the synthetic gallery-scan save flow through cold restart and airplane-mode recovery on the Pixel 7 emulator.
...
- [x] Verify primary destinations, task creation/details, and permission recovery at 130% and 150% Android font scaling.
- [~] Add formal TalkBack testing to every completed flow. Primary navigation, manual Add Task, Task Details, task cards, Calendar empty state, notification explanation recovery, blank validation, and unsaved-change protection pass on the Pixel 7; Scan processing/review and conditional cloud states remain.
