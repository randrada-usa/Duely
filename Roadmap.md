...
Next recommended task: complete formal TalkBack testing and the remaining recovery-state matrix, then perform the final DuelyPages/DuelyIcons consistency review. Passwordless school-email authentication and the verified-school badge are deliberately deferred. The real Nunito and Inter font families now load before app startup. Pixel 7 verification confirmed local task retention through sign-out, successful Google sign-in to a second account, separate cloud ownership, an explicit first-backup prompt without silent copying, explicit empty-device restore, and a keep-data uninstall/reinstall that retained all 47 app-data files, the local task, and the signed-in Google session. Queued offline retry, ongoing subject/reminder synchronization, and the corrected 12-image OCR pack are also live-verified on Android.
...
- [ ] Add formal TalkBack testing to every completed flow.
