...
4. Restart Metro after changing `.env.local` so Expo rebuilds the JavaScript bundle with the new public configuration. Open Profile: **Sign in with Google** should be directly below the Guest header. After signing in, **Sign out** appears there instead.
